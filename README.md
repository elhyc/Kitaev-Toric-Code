...
Now suppose that we had a path of  bit flip errors, traversing through adjacent plaquettes. Then, it is easy to see that each plaquette on the path will have an even number of $-1$ eigenvector qubits around their respective boundaries, except for the starting plaquette of the path and the ending plaquette of the path. In other words, $-1$ eigenvalue measurements among plaquette operators can be "pushed out" along a path of bit flips, and the total number of plaquettes operators with $-1$-eigenvalue measurements will always be an even number.  